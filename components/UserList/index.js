  ...
  useEffect(() => {
    const filteredUsers = () => {
      let filteredData = users.filter(
        user =>
          user.name.toLowerCase().includes(searchName.toLowerCase()) && user.email.toLowerCase().includes(searchEmail.toLowerCase())
      ).sort((a, b) => {
...
const UserList = () => {
  const { users, columnFields, handleOnSearch, handleSort, sortColumn, sortDirection } = WithUserData();
